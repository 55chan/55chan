...
if ($requestMethod === "POST"){
  ...
  if ($resp === '1') {
    ...
    if (!$tor) {
      $query = prepare('INSERT INTO ``dose_diaria`` VALUES(:ip, NOW(), 0) ON DUPLICATE KEY UPDATE `created`=NOW(),`uses`=0');
      $query->bindValue(':ip', $identity);
      $query->execute() or error(db_error($query));
    } else {
	error("Nope, wait a bit more");
    }

// Let's leave it commented for now
...
*/	
    ...
    $errorText = _('You failed the CAPTCHA') . _('. <a href="/dnsbls_bypass.php">Try again.</a> If it\'s not working, it can\'t be helped.');
