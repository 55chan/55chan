	...
	if (window.Options && window.Options.get_tab("general")){
        ...
        var clickEmbedButton = function () {
            ...
            if ($(this).attr("data-opened") == "false") {
                ...
                $(this).children('a').text("Fechar");
            } else {

                var a = $.find("[data-ytid='" + yt_id + "']" + "[data-ytind='" + yt_ind + "']");
                $(a).remove();
                $(this).attr("data-opened", "false");
                $(this).children('a').text("Embutir");
            }
		...
		Options.extend_tab('general', '<label id="toggle-emb-vid"><input type="checkbox">Embutir links do YouTube</label>');
