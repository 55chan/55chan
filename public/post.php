...
if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['delete'])) {
        ...
        if ($post['has_file']) {
            ...
            foreach ($_FILES as $key => $file) {
                ...
                if ($file['size'] && $file['tmp_name']) {
                    ...
                    $file['file'] = $config['public'] . $board['dir'] . $config['dir']['img'] . $file['file_id'] . '.' . $file['extension'];
                    $file['thumb'] = $config['public'] . $board['dir'] . $config['dir']['thumb'] . $file['file_id'] . '.' . ($config['thumb_ext'] ? $config['thumb_ext'] : $file['extension']);
                    $post['files'][] = $file;
                    $i++;
                }
            }
        ...
        if ($post['has_file']) {
            foreach ($post['files'] as $key => &$file) {
                ...
                $file['file'] = mb_substr($file['file'], mb_strlen($config['public'] . $board['dir'] . $config['dir']['img']));
                if ($file['is_an_image'] && $file['thumb'] != 'spoiler')
                    $file['thumb'] = mb_substr($file['thumb'], mb_strlen($config['public'] . $board['dir'] . $config['dir']['thumb']));
            }
        }
	...
	!file_exists($config['has_installed']) ? header('Location: install.php', true, $config['redirect_http']) : error($config['error']['nopost']);
}
